...
//    int bound_num;
//
//    char filename[20] = "fore_all.dat";
//    readinput(filename, &node_num, &fore_all);
//    char filename2[20] = "bound_all.dat";
...
//    }
//
//    double* dist_map = (double*)malloc(x_size * y_size * z_size * sizeof(double));
//    
//    for (int ii = 0; ii < x_size; ii++) {
//        for (int jj = 0; jj < y_size; jj++) {
//            for (int kk = 0; kk < z_size; kk++) {
...
//                    direction[direction_ind][3] = abs(xx) + abs(yy) + abs(zz) - 1;
//                    direction_ind++;
//                }
//            }
//        }
...
//                                node_capacity[node_s * 26 + node_edge_num[node_s]] = direction[dd][3];
...
//    int a = 1;
//// variables in loop:
//// dynamic: bd node_ind node_list node_flag node_dist edge_queue edge_queue_time node_queue
//// static : node_list_num node_s node_t edgeind edgeind_new nodeind current_time fire_time
//
//    omp_set_num_threads(num_threads);
//#pragma omp parallel private(node_s, node_t)
//    {
//        int b = omp_get_thread_num();
//        printf("%d\n", b);
//        int step = (win_size - 1) / 2;
//        int center[3] = { step,step,step };
//        float c[3] = { 1,sqrt(2),sqrt(3) };
//        float inf = 1e30;
//
//        struct body bd;
//        bd.numpoints = 0;
//        bd.coord = (double**)malloc(win_size * win_size * win_size * sizeof(double*));
//        for (int ii = 0; ii < win_size * win_size * win_size; ii++) {
//            bd.coord[ii] = (double*)malloc(3 * sizeof(double));
//        }
//
//        int** node_ind = (int**)malloc(3 * sizeof(int*));
//        for (int ii = 0; ii < 3; ii++) {
//            node_ind[ii] = (int*)malloc(win_size * win_size * win_size * sizeof(int));
//        }
//        int* node_list = (int*)malloc(win_size * win_size * win_size * sizeof(int));
//
...
//            edge_queue[ii] = (int*)malloc(26 * win_size * win_size * win_size * sizeof(int));
//            edge_queue_time[ii] = (float*)malloc(26 * win_size * win_size * win_size * sizeof(float));
//        }
//        int* node_queue = (int*)malloc(win_size * win_size * win_size * sizeof(int));
//        
//        int bound_ind;
//#pragma omp for
...
//                        node_s = fore_all[bound_list[0][bound_ind] - step + ii]
//                            [bound_list[1][bound_ind] - step + jj][bound_list[2][bound_ind] - step + kk];
//                        if (node_s > 0) {
...
//            float fire_time;
//            edgeind = 0;
//            node_s = fore_all[bound_list[0][bound_ind] - step + center[0]]
//                [bound_list[1][bound_ind] - step + center[1]][bound_list[2][bound_ind] - step + center[2]];
//            node_dist[node_s] = 0;
...
//                if (node_dist[node_list[ii]] > step - 0.5 && node_dist[node_list[ii]] < step + 0.5) {
//                    bd.coord[bd.numpoints][0] = (double)node_ind[0][ii] - center[0];
//                    bd.coord[bd.numpoints][1] = (double)node_ind[1][ii] - center[1];
//                    bd.coord[bd.numpoints][2] = (double)node_ind[2][ii] - center[2];
//                    bd.numpoints++;
//                }
...
//                dist_map[bound_list[2][bound_ind] * x_size * y_size + bound_list[1][bound_ind] * x_size +
//                    bound_list[0][bound_ind]] = distance;
//            }
//
//        }
//
...
void mexFunction(int nlhs, mxArray* plhs[],
    ...
    // £¨node_num, fore_all, bound_num, bound_all, win_size)
    ...
    const mwSize dims_output[3] = { x_size, y_size, z_size };
    plhs[0] = mxCreateNumericArray(3, dims_output, mxDOUBLE_CLASS, mxREAL);
    ...
    for (int ii = 0; ii < x_size; ii++) {
        for (int jj = 0; jj < y_size; jj++) {
            for (int kk = 0; kk < z_size; kk++) {
                ...
                dist_map[kk * x_size * y_size + jj * x_size + ii] = NAN;
            }
        }
    }
